...
def adjust_audio_speed(input_file: str, output_file: str, speed_factor: float) -> None:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if (
                output_duration >= expected_duration * 1.02
                and input_duration < 3
                and diff <= 0.1
            ):
                ...
                print(
                    f"✂️ Trimmed to expected duration: {expected_duration:.2f} seconds"
                )
                return
            elif output_duration >= expected_duration * 1.02:
                raise Exception(
                    f"Audio duration abnormal: input file={input_file}, output file={output_file}, speed factor={speed_factor}, input duration={input_duration:.2f}s, output duration={output_duration:.2f}s"
                )
            return
        except subprocess.CalledProcessError as e:
            if attempt < max_retries - 1:
                rprint(
                    f"[yellow]⚠️ Audio speed adjustment failed, retrying in 1s ({attempt + 1}/{max_retries})[/yellow]"
                )
                time.sleep(1)
            else:
                rprint(
                    f"[red]❌ Audio speed adjustment failed, max retries reached ({max_retries})[/red]"
                )
                raise e
...
def generate_tts_audio(tasks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    with Progress() as progress:
        ...
        max_workers = (
            load_key("max_workers") if load_key("tts_method") != "gpt_sovits" else 1
        )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        tasks_df.loc[tasks_df["number"] == number, "real_dur"] = (
                            real_dur
                        )
                        progress.advance(task)
                    except Exception as e:
...
def process_chunk(
    chunk_df: pd.DataFrame, accept: float, min_speed: float
) -> tuple[float, bool]:
    ...
    if (chunk_durs + all_gaps) / accept < durations:
        speed_factor = max(
            min_speed, (chunk_durs + all_gaps) / (durations - speed_var_error)
        )
        ...
        speed_factor = max(
            min_speed, (chunk_durs + all_gaps) / (tol_durs - speed_var_error)
        )
    else:
    ...
    for index, row in tasks_df.iterrows():
        if row["cut_off"] == 1:
            ...
            for i, row in chunk_df.iterrows():
                ...
                rprint(
                    f"[cyan]{emoji} Processed chunk {chunk_start} to {index} with speed factor {speed_factor}[/cyan]"
                )
            ...
            if cur_time > chunk_end_time:
                ...
                if (
                    time_diff <= 0.6
                ):  # If exceeding time is within 0.6 seconds, truncate the last audio
                    rprint(
                        f"[yellow]⚠️ Chunk {chunk_start} to {index} exceeds by {time_diff:.3f}s, truncating last audio[/yellow]"
                    )
                    # Get the last audio file
                    last_number = tasks_df.iloc[index]["number"]
                    last_lines = (
                        eval(tasks_df.iloc[index]["lines"])
                        if isinstance(tasks_df.iloc[index]["lines"], str)
                        else tasks_df.iloc[index]["lines"]
                    )
                    last_line_index = len(last_lines) - 1
                    last_file = OUTPUT_FILE_TEMPLATE.format(
                        f"{last_number}_{last_line_index}"
                    )
                    ...
                    raise Exception(
                        f"Chunk {chunk_start} to {index} exceeds the chunk end time {chunk_end_time:.2f} seconds with current time {cur_time:.2f} seconds"
                    )
            chunk_start = index + 1
