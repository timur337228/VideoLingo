...
def analyze_subtitle_timing_and_speed(df):
    ...
    for i in range(len(df) - 1):
        current_end = datetime.datetime.strptime(
            df.loc[i, "end_time"], "%H:%M:%S.%f"
        ).time()
        next_start = datetime.datetime.strptime(
            df.loc[i + 1, "start_time"], "%H:%M:%S.%f"
        ).time()
        df.loc[i, "gap"] = time_diff_seconds(
            current_end, next_start, datetime.date.today()
        )
...
def process_cutoffs(df):
    ...
    df.loc[df["gap"] >= load_key("tolerance"), "cut_off"] = (
        1  # Set to 1 when gap is greater than TOLERANCE
    )
    idx = 0
    while idx < len(df):
        ...
        if df.iloc[idx]["cut_off"] == 1:
            if df.iloc[idx]["if_too_fast"] == 2:
                rprint(
                    f"[⚠️ Warning] Line {idx} is too fast and cannot be fixed by speed adjustment"
                )
            idx += 1
            continue
...
def gen_dub_chunks():
    ...
    for block in content.strip().split("\n\n"):
        ...
        if len(lines) >= 3:
            ...
            times = lines[1].split(" --> ")
            dt1 = datetime.datetime.strptime(times[0], "%H:%M:%S,%f")
            dt2 = datetime.datetime.strptime(times[1], "%H:%M:%S,%f")

            dur_content_lines.append((dt2 - dt1).total_seconds())
    ...
    for idx, row in df.iterrows():
        ...
        for i in range(last_idx, len(content_lines)):
            ...
            durs.append(dur_content_lines[i])
